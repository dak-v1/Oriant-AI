"use client";
/**
 * SummaryStep — onboarding step 4 (spec §7.2): the seven capture areas as a
 * checklist (captured vs "Discovery will cover this"), Edit affordances that
 * jump back to the owning step, and the permissions & consent card.
 */
import { motion, useReducedMotion } from "framer-motion";
import { Check, Minus, Pencil, ShieldCheck } from "lucide-react";
...
export default function SummaryStep({
  ...
  const snippetFor = (sectionId: string): string | null => {
    switch (sectionId) {
      case "company":
        return onboarding.intro.trim() ? `“${clip(onboarding.intro.trim(), 110)}”` : null;
  ...
  return (
    <div style={{ display: "grid", gap: 22 }}>
      <div style={{ display: "grid", gap: 6 }}>
        <h2 className="oa-h3">Here’s what Oriant has so far</h2>
        <p className="oa-sub">
      ...
      <ul className={styles.checkList}>
        {ONBOARDING_SECTIONS.map((sec, i) => {
          const captured = onboarding.capturedSections.includes(sec.id);
          const snippet = captured ? snippetFor(sec.id) : null;
          ...
          return (
            ...
            >
              <span
                className={captured ? styles.checkIconDone : styles.checkIconTodo}
                aria-hidden
              >
                {captured ? <Check size={13} /> : <Minus size={13} />}
              </span>
              <div>
                <div className={styles.checkTitleRow}>
                  ...
                  {!captured && (
                    <span className="oa-tag oa-tag--neutral">
                      Discovery will cover this
                    </span>
                  )}
                </div>
                <p className="oa-sub">{snippet ?? sec.blurb}</p>
              </div>
              {captured && stepIdx !== undefined && (
                <button
                  type="button"
        ...
        <ul className={styles.consentLines}>
          <li>
            ...
            <span>
              ...
              approve. Today’s selections connect nothing.
            </span>
          </li>
          <li>
            <ShieldCheck size={15} aria-hidden />
